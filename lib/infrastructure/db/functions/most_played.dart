...
List<AllSongsLists> mostPlayedListBloc = [];
...
Future<void> getMostPlayed() async {
  final mostPlayedDB = await Hive.openBox<AllSongsLists>('most_played_db');
  mostPlayedListBloc.clear();
  mostPlayedListBloc.addAll(mostPlayedDB.values);
  // mostPlayedNotifer.value.clear();
  // mostPlayedNotifer.value.addAll(mostPlayedDB.values);
  AllSongsLists temp;
  for (var i = 0; i < mostPlayedListBloc.length; i++) {
    for (var j = i + 1; j < mostPlayedListBloc.length; j++) {
      if (mostPlayedListBloc[i].count! < mostPlayedListBloc[j].count!) {
        temp = mostPlayedListBloc[i];
        mostPlayedListBloc[i] = mostPlayedListBloc[j];
        mostPlayedListBloc[j] = temp;
      }
    }
  }
